  ...
  return purchaseTransaction(
    ...
    async (session) => {
      ...
      if (fromCart) {
        ...
        const inCart = (cart?.items || []).map(
          (item) => String(item.course)
        );
      ...
      // --------------------------------------------------
      // Remove courses the student already owns
      // --------------------------------------------------
      ...
      const ownedIds = new Set(
        owned.map((item) => String(item.course))
      );
      ...
      if (!ids.length) {
        throw policyError(
          409,
          "You are already enrolled in the selected courses"
        );
      }
      ...
      const courses = await enrollCourses(
        studentId,
        ids,
        session,
        {
      ...
      // --------------------------------------------------
      // Create server-side price snapshot
      // --------------------------------------------------
        ...
        body: {
          message:
            totalAmount === 0
              ...
              : "Order created. Continue to Stripe to complete your payment.",
