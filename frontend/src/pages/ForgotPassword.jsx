...
export default function ForgotPassword() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || "Unable to send the reset link. Please try again later.");
      }
      setMessageType("success");
      setMessage(data.message || "If an account exists for that email, a password reset link has been sent.");
    } catch (error) {
      setMessageType("error");
      setMessage(error.message || "Unable to contact the server. Please try again later.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main className="password-recovery-page">
      <section className="password-recovery-card">
        ...
        {message && <p className={`password-recovery-message ${messageType}`} role={messageType === "error" ? "alert" : "status"}>{message}</p>}
