...
export default function Subscription() {
  ...
  return <main className="student-profile-page"><div className="student-profile-container subscription-page">
    ...
    {subscription?.pendingPayment && <section className="subscription-summary" role="status"><p>{subscription.pendingPayment.status === "awaiting_verification" ? "Premium payment submitted — awaiting admin approval." : subscription.pendingPayment.status === "rejected" ? "Premium payment rejected — review the reason and submit a new slip." : "You have a Premium checkout awaiting payment."}</p><Link className="subscription-button" to={`/checkout?orderId=${encodeURIComponent(subscription.pendingPayment._id)}&from=subscription`}>View Premium payment</Link></section>}
