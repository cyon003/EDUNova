...
function UnderstandingCheck({ course, lesson }) {
  ...
  const checkUnderstanding = (event) => {
    ...
    const misconception = training.misconceptions.find((item) =>
      item.patterns.some((pattern) =>
        normalizedAnswer.includes(pattern)
      )
    );

    const understoodConcepts = training.concepts.filter(
      (concept) => normalizedAnswer.includes(concept)
    );

    const missingConcepts = training.concepts.filter(
      (concept) => !understoodConcepts.includes(concept)
    );
  ...
  return (
    <section
      className="understanding-check"
      aria-label={`Check understanding for ${lesson.title}`}
    >
      <header>
        <span>
          <FaBrain />
        ...
        <div>
          <small>AI UNDERSTANDING CHECK</small>
          <h4>Explain what you learned</h4>
        </div>
      </header>

      <p>
        Write two or three sentences. Your response will be checked
        for important ideas and common misunderstandings.
      </p>
      ...
      <form onSubmit={checkUnderstanding}>
        <textarea
          ...
          placeholder={`What did you understand about ${lesson.title}?`}
          rows="4"
        />

        <button type="submit" disabled={!answer.trim()}>
          <FaBrain /> Check my understanding
        </button>
      </form>

      {feedback && (
        <div
          className="understanding-feedback"
          aria-live="polite"
        >
          {feedback.misconception ? (
            <article className="needs-review">
              <strong>Possible confusion</strong>
              <p>{feedback.misconception}</p>
            </article>
          ) : (
            <article>
              <strong>
              ...
              <p>
                {feedback.understood.length
                  ? `You connected: ${feedback.understood.join(", ")}.`
                  : "Your explanation does not yet mention the lesson's main concepts."}
              </p>
            </article>
          )}
          ...
          {feedback.missing.length > 0 && (
            <article>
              <strong>Ideas to include</strong>
              <p>{feedback.missing.join(" · ")}</p>
            </article>
          )}
  ...
  return (
    ...
    >
      <header className="course-lessons-heading">
        ...
        <h2 id="course-lessons-title">
          Learn one step at a time
        </h2>

        <p>
          Watch every lesson at your own pace and return whenever
      ...
      {lessons.length === 0 ? (
        <div className="course-lessons-empty">
          ...
          <p>
            The instructor has not uploaded video lessons for this
            course yet.
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="course-lesson-video">
                  {locked ? (
                    ...
                    <div className="lesson-media-unavailable">
                      No lesson video has been uploaded.
                    </div>
                  )}
                </div>
...
function getCourseMissions(course) {
  const missionSets = {
    mathematics: [
      [
        "Foundation",
        "Plan a School Fundraiser",
        "Use variables and equations to calculate a realistic fundraising target.",
      ],
      [
        "Applied",
        "Price a Delivery Service",
        "Combine equations and graphs to create a fair delivery-pricing model.",
      ],
      [
        "Capstone",
        "Forecast a Pop-Up Store",
        "Apply the complete course to model costs, revenue, and profit.",
      ],
    ],
    ...
    science: [
      [
        "Foundation",
        "Investigate a Plant Problem",
        "Use scientific observation to identify why a plant is not growing well.",
      ],
      [
        "Applied",
        "Improve a Classroom Experiment",
        "Combine scientific methods to create a reliable testing plan.",
      ],
      [
        "Capstone",
        "Advise a Sustainable Community",
        "Use the complete course to investigate and explain an environmental problem.",
      ],
    ],
    ...
    english: [
      [
        ...
        "Apply grammar and tone to make an unclear message professional.",
      ],
      [
        "Applied",
        "Create a Product Story",
        "Combine audience, structure, and persuasive writing skills.",
      ],
      [
        "Capstone",
        "Lead a Communication Campaign",
        "Plan, write, revise, and defend a complete campaign.",
      ],
    ],
  };
  ...
  const fallback = [
    [
      "Foundation",
      `Apply ${course.name} Foundations`,
      "Use the first part of the course to solve a guided real-world problem.",
    ],
    [
      "Applied",
      `Use ${course.name} in Practice`,
      "Combine knowledge from the first two parts of the course in a realistic situation.",
    ],
    [
      "Capstone",
      `${course.name} Final Challenge`,
      "Apply the complete course to a complex portfolio-ready problem.",
    ],
  ];

  const selected =
    missionSets[course.slug] || fallback;

  return selected.map(
    ([stage, title, description], index) => ({
...
function CourseMissions({
  ...
  return (
    <section
      className="course-missions"
      aria-labelledby="course-missions-title"
    >
      <header className="course-missions-heading">
        <div>
          <span>AI MISSION CHECKPOINTS</span>

          <strong>
            {completedLessonCount}/{lessonsCount} lessons complete
          </strong>
        </div>

        <h2 id="course-missions-title">
          Apply what you have learned
        </h2>

        <p>
          New missions unlock after one-third, two-thirds, and
          full course completion.
        </p>
      </header>

      <div className="course-mission-progress">
        <span
          style={{
            width: `${Math.min(
              (completedLessonCount /
      ...
      <div className="course-mission-grid">
        {missions.map((mission) => {
          const requiredLessons = Math.ceil(
            (lessonsCount *
              mission.fraction) /
              3
          );
          ...
          return (
            <article
              ...
              key={mission.id}
            >
              <div className="course-mission-icon">
                {complete ? (
                  <FaCheck />
                ) : unlocked ? (
                  <FaBrain />
              ...
              <div>
                <span>
                  <FaClock /> {mission.duration}
                </span>

                <span>
                  <FaStar /> {mission.difficulty}
                </span>
              </div>
      ...
      {selectedMission && (
        <div className="course-mission-overlay">
          <section
            className="course-mission-modal"
            role="dialog"
            aria-modal="true"
          ...
          >
            <header>
              <div>
                <small>
                  {selectedMission.stage} Mission
                </small>

                <strong id="active-mission-title">
            ...
            {submitted ? (
              <div className="course-mission-result">
                <span>
                  <FaTrophy />
                </span>
                ...
                <p>
                  You connected course knowledge to the scenario
                  and explained a practical solution. Full AI
                  scoring and personalized feedback will be
                  connected later.
                </p>
                ...
                <div>
                  ...
                  <span>
                    Reasoning · Application · Communication
                  </span>
                </div>
              ...
              <div className="course-mission-workspace">
                <div>
                  ...
                  <section>
                    ...
                    <div>
                      ...
                      <p>
                        "Explain what information you need from
                        me before you begin."
                      </p>
                    </div>
                  </section>
                ...
                >
                  ...
                  {hintVisible && (
                    <p className="course-mission-hint">
                      Identify what the client needs, connect it
                      to the course concepts, and explain every
                  ...
                  <textarea
                    ...
                    placeholder="Write your reasoning and final solution..."
                    required
                  />

                  <button type="submit">
                    <FaSave /> Submit Mission
                  </button>
                </form>
              </div>
            )}
...
function CourseDetail() {
  ...
  const enrollmentStorageKey =
    currentUser?.id
      ? `edunova-enrolled-courses-${currentUser.id}`
      : null;
        ...
        const storedCourses =
          JSON.parse(
            localStorage.getItem(
              `edunova-enrolled-courses-${user.id}`
            )
          );
      ...
      async () => {
        try {
          ...
          if (
            ...
            const syncResponse =
              await fetch(
                `${API_ROOT}/enrollments/${courseSlug}`,
                {
                  method: "POST",
                  headers: {
                    Authorization: `Bearer ${token}`,
            ...
            if (syncResponse.ok) {
              ...
              if (
                ...
                const progressResponse =
                  await fetch(
                    `${API_ROOT}/enrollments/${courseSlug}/progress`,
                    {
                      method: "PATCH",
                      headers: {
                        "Content-Type":
  ...
  if (!course) {
    return (
      <main className="course-detail-page">
        ...
        <p>
          This course may be awaiting approval or is no longer
          public.
        </p>
  ...
  // --------------------------------------------------
  // Enroll free course OR start direct Buy Now payment
  // --------------------------------------------------
  const enrollCourse = async () => {
    ...
    try {
      ...
      if (isFree) {
        const response =
          await fetch(
            `${API_ROOT}/enrollments/${course.slug}`,
            {
              method: "POST",
              headers: {
                Authorization: `Bearer ${token}`,
        ...
        setEnrollmentMessage(
          "Enrollment saved to your account."
        );
      ...
      // ----------------------------------------------
      // PAID COURSE — DIRECT BUY NOW
      // ----------------------------------------------
      //
      // IMPORTANT:
      // This does NOT add the course to the cart.
      //
      // The backend gets the real course price from MongoDB
      // and creates the pending order.

      const response =
        await fetch(
          `${API_ROOT}/orders/buy-now`,
          {
            method: "POST",
            headers: {
              "Content-Type":
      ...
      //
      // The order ID lets the payment page reopen
      // the exact order after refresh or leaving the page.
  ...
  const saveProgress = async (
    ...
    try {
      ...
      let response =
        await fetch(
          `${API_ROOT}/enrollments/${course.slug}/progress`,
          options
        );
      ...
      if (!response.ok) {
        throw new Error(
          "Unable to synchronize course progress"
        );
      }
    } catch (error) {
      console.error(
  ...
  return (
    <main className="course-detail-page">
        ...
        <article className="course-detail-card">
          ...
          <div className="course-detail-copy">
            ...
            <div className="course-enrollment-note">
              <FaCheck /> Learn at your own pace with lifetime
              access
            </div>
      ...
      {reportOpen && (
        <div
          className="course-report-overlay"
          role="presentation"
          onMouseDown={() =>
            setReportOpen(false)
          }
        >
          ...
          >
            ...
            {reportMessage && (
              <p className="course-report-message">
                {reportMessage}
              </p>
            )}
            ...
            <label>
              ...
              <textarea
                ...
                placeholder="Describe the problem clearly so the admin can investigate it."
              />
            </label>
