...
export function establishSession(user, token) {
  ...
  // Legacy callers use this only as an authenticated-state marker. The global
  // request wrapper replaces it with the short-lived token held in memory.
...
async function authenticatedFetch(input, init = {}, mayRetry = true) {
  ...
  if (apiRequest) {
    ...
    else if (headers.get("Authorization") === `Bearer ${TOKEN_MARKER}`) headers.delete("Authorization");
  }
  options.headers = headers;

  const response = await getNativeFetch()(input, options);
  if (response.status !== 401 || !apiRequest || !mayRetry || isRefreshExcluded(input)) {
    return response;
  }
...
export async function logout() {
  try {
    await getNativeFetch()(`${AUTH_ROOT}/logout`, { method: "POST", credentials: "include" });
  } finally {
    clearSession();
  }
}
...
export async function logoutAll() {
  try {
    await authenticatedFetch(`${AUTH_ROOT}/logout-all`, { method: "POST" });
  } finally {
    clearSession();
  }
}
